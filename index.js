...
app.post("/bank_nifty_option_chain", fetchUser, handleDataRoute(BankNiftyOptionData));

// Fin Nifty Option Chain Data Route
app.post("/fin_nifty_option_chain", fetchUser, handleDataRoute(FinNiftyOptionData));
...
app.post("/bank_nifty_market_price", fetchUser, handleDataRoute(BankNiftyMarketPrice));
...
const setLiveMarketPrice = async (marketPriceModel, getPriceFn) => {
  try {
    ...
    const marketData = await marketPriceModel.find().sort({ _id: -1 }).limit(1).exec();
...
const fetchData = async () => {
  ...
  const [niftyPrice, bankNiftyPrice, finNiftyPrice] = await Promise.all([getNiftyPrice(), getBankNiftyPrice(), getFinNiftyPrice()]);
  ...
  const bankNiftyData = calculateData(bankNiftyResult, bankNiftyPrice, currentDate);
  const finNiftyData = calculateData(finNiftyResult, finNiftyPrice, currentDate);
...
const calculateData = (result, price, currentDate) => {
  ...
  result.forEach((item) => {
    data.TotalCallLTP += item.CallLTP === '-' ? 0 : parseFloat(item.CallLTP.replace(/,/g, ''));
    data.TotalCallChgLTP += item.CallChgLTP === '-' ? 0 : parseFloat(item.CallChgLTP.replace(/,/g, ''));
    data.TotalCallVol += item.CallVol === '-' ? 0 : parseFloat(item.CallVol.replace(/,/g, ''));
    data.TotalCallOI += item.CallOI === '-' ? 0 : parseFloat(item.CallOI.replace(/,/g, ''));
    data.TotalCallChgOI += item.CallChgOI === '-' ? 0 : parseFloat(item.CallChgOI.replace(/,/g, ''));
    data.TotalPutLTP += item.PutLTP === '-' ? 0 : parseFloat(item.PutLTP.replace(/,/g, ''));
    data.TotalPutChgLTP += item.PutChgLTP === '-' ? 0 : parseFloat(item.PutChgLTP.replace(/,/g, ''));
    data.TotalPutVol += item.PutVol === '-' ? 0 : parseFloat(item.PutVol.replace(/,/g, ''));
    data.TotalPutOI += item.PutOI === '-' ? 0 : parseFloat(item.PutOI.replace(/,/g, ''));
    data.TotalPutChgOI += item.PutChgOI === '-' ? 0 : parseFloat(item.PutChgOI.replace(/,/g, ''));
  });
...
const fetchMarketData = async () => {
  ...
  const isMarketOpen =
    ...
    now.day() <= 5 &&
    now.isBetween(
      moment.tz("Asia/Kolkata").hour(9).minute(15),
